#!/usr/bin/env python3
"""
Morning Post Generator: Theory + Edge Cases + Code Examples
Generates learning content for morning consumption (25 slides)
"""
...
def measure_actual_content_height(content_html, css, driver, section_name, topic_name):
    ...
    css_for_measurement = css.replace(
        ...
        '/* Section containers should grow to fill space - ONLY inside slide-content */\n    .slide-content .section-theory,\n    .slide-content .section-edge-cases,\n    .slide-content .section-code-examples {\n        flex: 1;\n        display: flex;\n        flex-direction: column;\n        overflow: hidden;\n    }',
...
def split_content_with_measurement(content_md, section_name, topic_name, css, driver):
    ...
    """
    MAX_HEIGHT = 1020  # Leaves 60px margin at bottom for safety
...
<style>
    * {
    ...
    .topic-header.section-theory {
        border-bottom: 3px solid #3b82f6;
    }

    .topic-header.section-edge-cases {
        border-bottom: 3px solid #f97316;
    }

    .topic-header.section-code-examples {
    ...
    .section-theory .section-name {
        color: #3b82f6;
    }

    .section-edge-cases .section-name {
        color: #f97316;
    }

    .section-code-examples .section-name {
        color: #10b981;
    }
    ...
    .section-theory .concept-title {
        background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%);
    }

    .section-edge-cases .concept-title {
        background: linear-gradient(135deg, #f97316 0%, #ea580c 100%);
    }

    .section-code-examples .concept-title {
...
def generate_morning_post(topic, template_dir, output_dir):
    """Generate morning post: Theory + Edge Cases + Code Examples"""

    print("\n" + "="*80)
    print("🌅 MORNING POST: Learn C++ - " + topic['topic'])
    print("="*80)
...
"""
    ...
    template_path = os.path.join(template_dir, 'morning_template.html')
    with open(template_path, 'w', encoding='utf-8') as f:
        f.write(template_content)

    template = env.get_template('morning_template.html')
    ...
    # Theory
    theory = topic['theory']
    if 'subsections' in theory:
        for subsec in theory['subsections']:
            # DON'T split code blocks here - let measurement handle it
            subsections.append({
                'title': subsec['title'],
    ...
    for item in code_examples:
        ...
        subsections.append({
            'title': f"{item['example_number']}. {item['title']}",
            'content': content,
    ...
    # Title slide
    slides.append({
        'file': f'morning_slide_{slide_num:02d}_title.html',
        'data': {
            'theme': 'theory', 'header_right': 'Morning Learning',
            'slide_num': slide_num, 'total_slides': 0,
            'title': f'Morning: {topic["topic"]}',
            'slide_type': 'title',
            'session_name': topic['topic'],  # Topic name as main title
            'topic_name': 'Theory, Edge Cases & Code Examples',  # Section types as subtitle
    ...
    try:
        ...
        for subsec_idx, subsection in enumerate(subsections, 1):
            ...
            for slide_idx, content_html in enumerate(slide_htmls, 1):
                ...
                slides.append({
                    'file': f'morning_slide_{slide_num:02d}_subsec{subsec_idx}_{slide_idx}.html',
                    'data': {
                        'theme': 'theory', 'header_right': 'Morning Learning',
                        'slide_num': slide_num, 'total_slides': 0,
                        'title': f'Morning: {topic["topic"]}',
    ...
    slides.append({
        'file': f'morning_slide_{slide_num:02d}_summary.html',
        'data': {
            'theme': 'theory', 'header_right': 'Morning Learning',
    ...
    # Generate HTML
    morning_dir = os.path.join(output_dir, 'morning_learn')
    ...
    for file in ['styles.css', 'prism.css', 'prism.js', 'prism-cpp.js']:
        ...
        dst = os.path.join(morning_dir, file)
    ...
    for slide in slides:
        ...
        with open(os.path.join(morning_dir, slide['file']), 'w', encoding='utf-8') as f:
            f.write(html)
    ...
    html_files = sorted([f for f in os.listdir(morning_dir) if f.endswith('.html')])
    ...
    with sync_playwright() as p:
        ...
        for html_file in html_files:
            ...
            html_path = os.path.join(morning_dir, html_file)
            pdf_path = os.path.join(morning_dir, pdf_name)
    ...
    combined_pdf_path = os.path.join(morning_dir, 'morning_learn_complete.pdf')
    merger = PdfMerger()

    # Get all PDF files in order (excluding any previous combined file)
    pdf_files = sorted([f for f in os.listdir(morning_dir)
                       if f.startswith('morning_slide_') and f.endswith('.pdf')])

    for pdf_file in pdf_files:
        pdf_path = os.path.join(morning_dir, pdf_file)
    ...
    print(f"   📄 Combined PDF: morning_learn_complete.pdf (ready for LinkedIn!)")
    print(f"\n   📂 Output: {morning_dir}")

    return len(slides), morning_dir

def main():
    # Parse command-line arguments
    topic_index = 0  # default to topic 1
    if len(sys.argv) > 1:
        try:
            topic_index = int(sys.argv[1]) - 1  # Convert to 0-based index
            if topic_index < 0:
                print("❌ Topic number must be >= 1")
                sys.exit(1)
        except ValueError:
            print("❌ Invalid topic number. Usage: python3 generate_morning.py [TOPIC_NUMBER]")
            print("   Example: python3 generate_morning.py 2  # Generate for topic 2")
            sys.exit(1)

    topic = load_topic_data(topic_index)
    template_dir = os.path.join(os.path.dirname(__file__), 'templates')
    output_base = os.path.join(os.path.dirname(__file__), 'output', f'topic{topic_index + 1}')
    os.makedirs(output_base, exist_ok=True)

    slides_count, output_dir = generate_morning_post(topic, template_dir, output_base)

    print("\n" + "="*80)
    print("🎉 MORNING POST COMPLETE!")
    print("="*80)
